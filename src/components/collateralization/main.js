...
function IPCVDeposit(address) {
  return new ethers.Contract(address, IPCVDepositAbi, provider);
}
var CollateralizationOracle = new ethers.Contract(collateralizationOracle, CollateralizationOracleAbi, provider);
var Fei = new ethers.Contract(fei, IERC20Abi, provider);
...
class c extends Component {
  ...
  getTokenImage(symbol, depositAddress) {
    if (depositAddress === '0x5ae217dE26f6Ff5F481C6e10ec48b2cf2fc857C8')
      return <img className="token" src={imgd3}></img>;
    if (depositAddress === '0x24F663c69Cd4B263cf5685A49013Ff5f1C898D24')
      return <img className="token" src={imgd3}></img>;
    if (depositAddress === '0xA271fF86426c7fdAaAE72603e6Ce68c892d69ED7')
      return <img className="token" src={imgSteth}></img>;
  ...
  async refreshData() {
    ...
    const cgko = await $getJSON(
      'https://api.coingecko.com/api/v3/simple/price?ids=angle-protocol,balancer,curve-dao-token,tribe-2,convex-finance,tokemak,aave,compound-governance-token,liquity&vs_currencies=usd'
    );
    this.state.cgko = {};
    ...
    for (var i = 0; i < this.state.getTokensInPcv.length; i++) {
      var tokenAddress = this.state.getTokensInPcv[i];
      var oracle = new ethers.Contract(this.state.tokenToOracle[tokenAddress], IOracleAbi, provider);
    ...
    for (var i = 0; i < this.state.getTokensInPcv.length; i++) {
      this.log(
        ...
        i + 1,
        '/',
        this.state.getTokensInPcv.length,
        '[' + this.state.tokens[tokenAddress].symbol + ']'
      );
      var tokenAddress = this.state.getTokensInPcv[i];
      for (var j = 0; j < this.state.getDepositsForToken[tokenAddress].length; j++) {
        ...
        this.log(
          '  ' + depositLabel + '.resistantBalanceAndFei() // Loading data for deposit',
          j + 1,
          '/',
          this.state.getDepositsForToken[tokenAddress].length,
          '...'
        );
        var deposit = new ethers.Contract(depositAddress, IPCVDepositAbi, provider);
    ...
    const stableAssets = ['DAI', 'LUSD', 'VOLT', 'agEUR', 'RAI'];
    ...
    comp[today] =
      comp[today] ||
      this.state.pcvComposition.reduce((acc, cur) => {
    ...
    this.state.tribeInTreasury =
      (await tribe.balanceOf('0x8d5ED43dCa8C2F7dFB20CF7b53CC7E593635d7b9')).toString() / 1e18;
    ...
    this.state.deposits.sort(function (a, b) {
      return a.balanceUSD + a.fei < b.balanceUSD + b.fei ? 1 : -1;
    });
  ...
  async getTokenSymbol(address) {
    ...
    var token = new ethers.Contract(address, IERC20Abi, provider);
    return await token.symbol();
  }

  async getDepositDescription(address) {
    if (address == '0x24F663c69Cd4B263cf5685A49013Ff5f1C898D24')
      return 'This is usually empty, as D3pool LP tokens are moved to the Convex deposit most of the time (to earn CRV and CVX rewards).';
    if (address == '0x06dAcca04e201AD31393754E68dA04Dc14778Fa6')
      return "This PCV Deposit is used to account a fixed $ value for some assets that are not very liquid or don't have a reliable on-chain oracle (like INDEX), and FEI deployments that don't have a fixed contract address (like Ondo LaaS).";
    if (address == '0x7Eb88140af813294aEDce981b6aC08fcd139d408')
      return 'Optimistic Approval is a community-owned multisig that can queue some governance actions on the Optimistic Approval Timelock to be executed after 4 days of delay (instead of going for a full on-chain vote).';
    if (address == '0xec54148CbC47bFF8FCc5e04e5E8083aDb8aF9aD9')
      return "Float Protocol's Fuse pool was hacked due to a Uni-v3 Oracle manipulation.";
    if (address == '0x5ae217dE26f6Ff5F481C6e10ec48b2cf2fc857C8')
      return 'Stake LP tokens in Convex & earn CRV & CVX rewards.';
    if (address == '0x0961d2a545e0c1201B313d14C57023682a546b9D')
      return 'Deposit tokens in reactor & earn TOKE rewards.';
    if (address == '0x0735e14D28eD395048d5Fa4a8dbe6e6EB9fc0470')
      return 'Deposit tokens in lending market & earn interests + COMP rewards.';
    if (address == '0xfDe7077AAEcDaf2C4B85261Aa858c96A7E737a61')
      return 'Deposit tokens in lending market & earn interests + COMP rewards.';
    if (address == '0xB80B3dc4F8B30589477b2bA0e4EF2b8224bDf0a5')
      return 'Deposit tokens in lending market & earn interests.';
    if (address == '0x43Ef03755991056681F01EE2182234eF6aF1f658')
      return 'Deposit tokens in lending market & earn interests + stkAAVE rewards.';
    if (address == '0x1267B39c93711Dd374DEAB15e0127e4adB259BE0')
      return 'Deposit tokens in lending market & earn interests + stkAAVE rewards.';
    if (address == '0xFAc571b6054619053ac311dA8112939C9a374A85')
      return 'Deposit tokens in lending market & earn interests.';
    if (address == '0x98E5F5706897074a4664DD3a32eB80242d6E694B')
      return 'The Peg Stability Modules are used to defend a 1$ peg for FEI by allowing users to mint new FEI by providing assets, or redeem FEI for assets.';
    ...
    if (address == '0x3a1838Ac9EcA864054bebB82C32455Dd7d7Fc89c')
      return 'The C.R.E.A.M. markets got hacked by an Oracle manipulation attack. The protocol lost the 5M FEI deposited in CREAM, and got these CREAM tokens in compensation for the hack.';
    if (address == '0x89DfBC12001b41985eFAbd7dFCae6a77B22E4Ec3')
      return 'The protocol can mint new FEI to buyback TRIBE on the open market using a Balancer LBP that lasts one week.';
    if (address == '0x2c47Fef515d2C70F2427706999E158533F7cF090')
      return 'Turbo is a Tribe DAO product that allows other DAOs to mint FEI into approved ERC4626 strategies, turning their idle governance tokens into productive assets.';
    if (address == '0xE8633C49AcE655EB4A8B720e6b12F09Bd3a97812')
      return "This is usually empty, as LP tokens are moved to the Angle Protocol's gauges to earn ANGLE rewards.";
    if (address == '0xc5bb8F0253776beC6FF450c2B40f092f7e7f5b57')
      return "This is usually empty, as LP tokens are moved to Balancer's gauges to earn BAL rewards.";
    if (address == '0xcd1Ac0014E2ebd972f40f24dF1694e6F528B2fD4')
      return 'This is usually empty, as B-80BAL-20WETH are vote-locked to veBAL.';
    return '';
  }
  ...
  async getDepositPL(address, tokenValue, balance, balanceUSD, protocolFei) {
    ...
    if (address == '0xcd1Ac0014E2ebd972f40f24dF1694e6F528B2fD4') {
      var currentBalance =
        (await ERC20('0x5c6Ee304399DBdB9C8Ef030aB642B10820DB8F56').balanceOf(address)).toString() / 1e18;
      return (currentBalance / 104595.9 - 1) * balanceUSD;
    }
    // Convex CRV & CVX rewards
    if (address == '0x5ae217dE26f6Ff5F481C6e10ec48b2cf2fc857C8') {
      const crv =
        ((await ERC20('0xD533a949740bb3306d119CC777fa900bA034cd52').balanceOf(address)).toString() / 1e18) *
        this.state.cgko['CRV'];
      const cvx =
        ((await ERC20('0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B').balanceOf(address)).toString() / 1e18) *
        this.state.cgko['CVX'];
      return crv + cvx;
    }
    // Curve d3pool
    ...
    if (address == '0x0961d2a545e0c1201B313d14C57023682a546b9D') {
      const toke =
        ((await ERC20('0x2e9d63788249371f1DFC918a52f8d799F4a38C94').balanceOf(address)).toString() / 1e18) *
        this.state.cgko['TOKE'];
      return toke;
    }
    // ANGLE tokens farmed
    if (address == '0xD2554839c2e8a87Dd2CddD013EF828B6534aBC26') {
      return (
        ((await IPCVDeposit('0xb91F96b7C62fe4a2301219956Cc023fA7892F0C0').balance()).toString() / 1e18) *
        this.state.cgko['ANGLE']
      );
    }
    // Compound ETH
    if (address == '0x0735e14D28eD395048d5Fa4a8dbe6e6EB9fc0470') {
      ...
      const comp =
        (await ERC20('0xc00e94Cb662C3520282E6f5717214004A7f26888').balanceOf(unwrappedDeposit)).toString() / 1e18;
    ...
    if (address == '0x43Ef03755991056681F01EE2182234eF6aF1f658') {
      ...
      const interest = balance - (await depositMinusWithdrawals(unwrappedDeposit));
      const stkAAVE =
        (await ERC20('0x4da27a545c0c5B758a6BA100e3a049001de870f5').balanceOf(unwrappedDeposit)).toString() / 1e18;
    ...
    // DAI PSM
    if (address == '0x2A188F9EB761F70ECEa083bA6c2A40145078dfc2') {
      var contract = new ethers.Contract(address, FixedPricePSMAbi, provider);
    ...
    return (
      <div className="collateralization">
        <div className="card section">
          ...
          <div className="info">
            <p>
              This page is a simple web tool that reads the{' '}
              <a href="https://etherscan.io/address/0xFF6f59333cfD8f4Ebc14aD0a0E181a83e655d257#code" target="_blank">
                Collateralization Oracle
              </a>{' '}
              of Fei Protocol, a smart contract that details where all the PCV assets that back the FEI stablecoin are
              deployed.
            </p>
            <p>
              Additional metadata are hard-coded in the front-end, such as the PCV Deposit description and protocol.
              Contract labels are fetched from{' '}
              <a
                href="https://github.com/fei-protocol/fei-protocol-core/blob/develop/protocol-configuration/mainnetAddresses.ts"
                target="_blank"
              >
                Github
              </a>
          ...
          {this.state.loading ? (
            <div className="info">
              ...
              <div className="text-center loading">
                Reading latest on-chain data (this can take several seconds / a minute)...
              </div>
            </div>
          ) : null}
          {!this.state.loading ? (
            <div>
              <table style={{ width: 'auto', margin: '0 0 1em', background: '#eee', float: 'left' }}>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <strong>{formatNumber(this.state.totalFei)}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>Protocol Controlled FEI</td>
                    <td>
                      <strong>{formatNumber(this.state.protocolFei)}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>User Circulating FEI</td>
                    <td>
                      <strong>{formatNumber(this.state.circulatingFei)}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>Protocol Controlled Value</td>
                    <td>
                      <strong>{formatNumber(this.state.pcv, '$ ')}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>Protocol Equity (PCV - Circulating FEI)</td>
                    <td>
                      <strong>{formatNumber(this.state.equity, '$ ')}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>Collateral Ratio</td>
                    <td>
                      <strong>{Math.round((100 * this.state.pcv) / this.state.circulatingFei)} %</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>Stable Backing ({this.state.stableAssets.join(', ')})</td>
                    <td>
                      <strong>{Math.round((100 * this.state.stableBacking) / this.state.circulatingFei)} %</strong>
                    </td>
                  </tr>
                  <tr>
                  ...
                  <tr>
                    ...
                    <td>
                      <strong>{formatNumber(this.state.tribeTotalSupply)}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>TRIBE in DAO Treasury</td>
                    <td>
                      <strong>{formatNumber(this.state.tribeInTreasury)}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>TRIBE Circulating (+ Vesting + LM)</td>
                    <td>
                      <strong>{formatNumber(this.state.tribeCirculating)}</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>TRIBE Circulating Market Cap</td>
                    <td>
                      <strong>{formatNumber(this.state.cgko['TRIBE'] * this.state.tribeCirculating, '$ ')}</strong>
                    </td>
                  </tr>
                </tbody>
              </table>
              <table style={{ width: 'auto', margin: '0 0 1em', background: '#eee', float: 'left', marginLeft: '1em' }}>
                <thead>
                  <tr>
                    <th colSpan="3">
                      <strong>Assets in the PCV</strong>
                    </th>
                  </tr>
                </thead>
                <tbody>
              ...
              <table style={{ width: 'auto', margin: '0 0 1em', background: '#eee', float: 'left', marginLeft: '1em' }}>
                ...
                <tbody>
                  {this.state.pcvProtocols.map((entry, i) => (
                    <tr key={i}>
                  ...
                  {this.state.deposits.map((deposit, i) => (
                    <tr key={i} className={i % 2 ? 'odd' : 'even'}>
                      ...
                      <td>
                        ...
                        {deposit.description ? (
                          <div className="text-muted text-small">{deposit.description}</div>
                        ) : null}
                      </td>
