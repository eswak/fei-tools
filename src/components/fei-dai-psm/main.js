    ...
    return (
      <div className="feidaipsm">
        <div className="card section">
          ...
          <div className="info">
            <p>
              The FEI-DAI <a href={'https://etherscan.io/address/' + psm.address} target="_blank">Peg Stability Module (PSM)</a> allows to mint 1 FEI by providing 1 DAI, and redeem 1 FEI for 1 DAI.
            </p>
            <p>
              This contract is immutable and will have the effect of pegging 1 FEI to the price of 1 DAI forever.
            </p>
          </div>
          ...
          <div className="box-wrapper">
            <div className="box">
              ...
              <div className="tabs">
                <div className={'tab ' + (this.state.tab == 'mint' ? 'active' : '')} onClick={()=>this.setActiveTab('mint')}>
                  Mint
                </div>
                <div className={'tab ' + (this.state.tab == 'redeem' ? 'active' : '')} onClick={()=>this.setActiveTab('redeem')}>
                  Redeem
                </div>
              </div>
              <div className="content">
                {
                  ...
                  <div className="input-box">
                    <input type="text" placeholder="0" value={this.state.input.fei} onChange={(e)=>this.onInputChange('fei', e)}/>
                    <span className="all" onClick={()=>this.setInputAmount('fei', this.state.balance.fei)} title={'Your balance: ' + formatNumber(this.state.balance.fei) + ' FEI'}>100%</span>
                    <span className="token"><img src={feiImg}/></span>
                  </div> :
                  <div className="input-box">
                    <input type="text" placeholder="0" value={this.state.input.dai} onChange={(e)=>this.onInputChange('dai', e)}/>
                    <span className="all" onClick={()=>this.setInputAmount('dai', this.state.balance.dai)} title={'Your balance: ' + formatNumber(this.state.balance.dai) + ' DAI'}>100%</span>
                    <span className="token"><img src={daiImg}/></span>
                  </div>
                }
                <div className="output-box">
                  <img src={ this.state.tab == 'redeem' ? arrow2Img : arrow1Img } className="arrow" /><br/>
                  <img src={ this.state.tab == 'redeem' ? daiImg : feiImg } className="token" />
                  <div className="out">
                    { this.state.tab == 'mint' ? formatNumber(this.state.input.dai * 1e18) : formatNumber(this.state.input.fei * 1e18)}
                  </div>
                </div>
                <div className="action-box">
                  {
                    this.state.tab == 'redeem' ?
                    <button disabled={this.state.allowance.fei / 1e18 >= this.state.input.fei} onClick={() => this.approveTx('fei')}>Approve FEI Transfer</button> :
                    <button disabled={this.state.allowance.dai / 1e18 >= this.state.input.dai} onClick={() => this.approveTx('dai')}>Approve DAI Transfer</button>
                  }
                  {
                    this.state.tab == 'redeem' ?
                    <button disabled={Number(this.state.input.fei) == 0 || this.state.allowance.fei / 1e18 < this.state.input.fei} onClick={() => this.redeemTx()}>Redeem</button> :
                    <button disabled={Number(this.state.input.dai) == 0 || this.state.allowance.dai / 1e18 < this.state.input.dai} onClick={() => this.mintTx()}>Mint</button>
                  }
                </div>
              </div>
            </div>
          ...
          <div className="events">
            ...
            <table style={{'maxWidth':'500px'}}>
              ...
              <tbody>
                {this.state.events.map((event, i) => <tr key={i}>
                  <td>
