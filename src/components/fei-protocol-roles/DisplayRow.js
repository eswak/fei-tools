...
class DisplayRow extends React.Component {
  ...
  render() {
    return (
      <tr key={this.props.rowkey} className={(this.props.rowkey % 2 ? 'odd' : 'even') + this.props.last}>
        <td>{this.props.rolelabel}</td>
        <td title="view account on etherscan">
          <a href={'https://etherscan.io/address/' + this.props.address} target="_blank">
            {this.props.label}
          </a>
        </td>
        {this.state.grantedDate == null ? (
        ...
        {this.props.revoked === true ? (
          <td className="text-center" title="view transaction on etherscan">
            {this.state.revokedDate === null ? (
              ...
              <a href={'https://etherscan.io/tx/' + this.props.revokeTransaction} target="_blank">
                {this.state.revokedDate}
              </a>
            )}
          </td>
        ) : null}
