...
export function ClaimAndRedeem(props) {
  ...
  const [inputValues, setInputValues] = useState(props.redeemableTokens.reduce((acc, cur) => {
    acc[cur.cToken] = '0';
    return acc;
  ...
  if (Object.keys(balances).length == 0) {
    ...
    promises.push(Promise.all(props.redeemableTokens.map((token) => {
    ...
    Promise.all(promises).then(function([_balances, _allowances, _userFei, _contractFei]) {
      props.redeemableTokens.forEach(function(token, i) {
        ...
        inputValues[token.cToken] = Math.floor(_balances[i].toString() / Math.pow(10, decimals[token.cToken])).toString();
      });
  ...
  const redeemedFei = Object.keys(inputValues || {}).map((cTokenAddress) => {
    return BigInt(Math.floor(inputValues[cTokenAddress] * Math.pow(10, decimals[cTokenAddress]) * rates[cTokenAddress] / 1e18)).toString();
  });
      ...
      <table className="mb-3">
        <thead>
          <tr>
            ...
            <th className="text-right" style={{'paddingRight':'5px'}}>Redeeming</th>
            <th width="0"></th>
            <th className="text-center">Approve</th>
          </tr>
        </thead>
        <tbody>
          {props.redeemableTokens.map((token, i) => <tr key={i} className={i % 2 ? 'odd' : 'even'}>
              ...
              <div style={{'position':'relative'}}>
                <div className="percents-box">
                  {[0, 33, 66, 100].map((percent, i) => {
                    return <div key={i} className="percent" onClick={()=>setPercent(token.cToken, percent)}>
                      {percent == 100 ? '%' : percent}
                    </div>;
                  })}
                </div>
                <input style={{'textAlign':'right','height':'30px'}} type="number" placeholder="0" value={inputValues[token.cToken] || ''} onChange={(e)=>handleTextInputChange(token.cToken, e.target.value)} />
              </div>
            </td>
            ...
            <td className="text-center">
              <button disabled={approved[i]} onClick={()=>doApprove(token.cToken)} title={'Current allowance ' + allowances[token.cToken]}>Approve</button>
            </td>
          </tr>)}
          <tr>
            <td></td>
            <td></td>
            <td></td>
            <td className="text-right" style={{'paddingRight':'5px'}}><strong>Total</strong></td>
            <td style={{'paddingLeft':'0'}}>= {formatNumber(totalRedeemedFei)} FEI</td>
          </tr>
        </tbody>
      </table>
      
      {!allApproved ? <div className="error mt-2">
        You must approve all cTokens before redeeming this amount.
      </div> : null}
      {!allBalanceAbove ? <div className="error mt-2">
        You must transfer more cTokens to your address before redeeming this amount.
      </div> : null}
      {totalRedeemedFei == '0' ? <div className="error mt-2">
        You cannot redeem 0 FEI.
      </div> : null}
      
      <div className="mt-2">
        <button disabled={totalRedeemedFei == '0' || !allBalanceAbove || !allApproved} onClick={()=>doRedeem()}>
          Redeem {formatNumber(totalRedeemedFei)} FEI in exchange of {amountsToRedeem.map((amountToRedeem, i) => {
            const cTokenAddress = cTokensToRedeem[i];
            return '[' + formatNumber(amountToRedeem, decimals[cTokenAddress]) + ' ' + labels[cTokenAddress] + ']';
          }).join(' + ')}
        </button>
      </div>
      <div className="mt-2">Your Balance: <img src={feiImg} style={{'height':'1em','verticalAlign':'-2px'}}/> {formatNumber(userFeiBalance)} FEI</div>
      <div className="mt-2">Redeemer Balance: <img src={feiImg} style={{'height':'1em','verticalAlign':'-2px'}}/> {formatNumber(contractFeiBalance)} FEI</div>
    </div>
  );
}
