  ...
  return (
    <tr className={props.rowkey % 2 ? 'odd' : 'even'}>
      <td title={props.cToken}>
        <a href={'https://etherscan.io/address/' + props.cToken}>{props.cTokenLabel}</a>
      </td>
      <td className="text-right" title={'Wei: ' + props.balance}>
        <a href={'https://etherscan.io/token/' + props.cToken + '?a=' + account}>{formatNumber(props.balance)}</a>
      </td>
      <td className="text-right" title={'Wei: ' + props.rate}>
        {formatRate(props.rate)}
      </td>
      <td className="text-right">{formatNumber(props.fei)}</td>
    </tr>
  );
}
