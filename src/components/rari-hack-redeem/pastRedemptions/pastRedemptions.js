...
export function PastRedemptions(props) {
    ...
    if (!userData.length) {
        Promise.all([ // ask the 3 event types in parallel
            getSignedEvents(),
            getClaimedEvents(),
            getRedeemedEvents()
        ]).then(function (data) {
            ...
            claimedEvents.forEach(function(claimedEvent) {
                userData[claimedEvent.args.claimant.toLowerCase()].claimed += rates[claimedEvent.args.cToken.toLowerCase()] / 1e18 * claimedEvent.args.claimAmount;
            });
            ...
            redeemedEvents.forEach(function(redeemedEvent) {
                userData[redeemedEvent.args.recipient.toLowerCase()].redeemed += rates[redeemedEvent.args.cToken.toLowerCase()] / 1e18 * redeemedEvent.args.cTokenAmount;
            });

            // array of user data sorted by claimable DESC
            setUserData(Object.values(userData).sort(function(a, b) {
                return a.claimable < b.claimable ? 1 : -1;
            }));
            // sum of all claimed
            setTotalClaimed(Object.values(userData).reduce(function(acc, cur) {
                acc += cur.claimed;
                return acc;
            }, 0));
            // sum of all claimable
            setTotalClaimable(Object.values(userData).reduce(function(acc, cur) {
                acc += cur.claimable;
                return acc;
            }, 0));
        });
    ...
    return (
        <div>
            <p>
                ...
                Amount of FEI left to claim {formatNumber(totalClaimable - totalClaimed)} ({formatPercent((totalClaimable - totalClaimed)/totalClaimable)})
            </p>
            <table className="mb-3" style={{'maxWidth':'900px'}}>
                ...
                <tbody>
                    {userData.map((d, i) => <tr key={i} className={i%2?'odd':'even'}>
                        ...
                        <td className="text-center">
                            {formatPercent(d.claimed / d.claimable)}
                        </td>
                        <td className="text-center">
                            {formatPercent(d.redeemed / d.claimable)}
                        </td>
                    </tr>)}
                </tbody>
            </table>
